class webbank_linux_android_slave {
  ...
  require  => Exec['mkdir apps sub dir'],
    path     => ['/usr/bin','/usr/sbin','/bin'],
  command  => 'wget --auth-no-challenge --http-user=jenkins --http-password=jenkins "http://10.6.222.54:8080/jenkins/job/mumble/lastSuccessfulBuild/artifact/dist/mumble.tgz"; chown -R rdm:rdm mumble.tgz';
  }
